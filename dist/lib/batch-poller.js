...
export class BatchPoller extends EventEmitter {
    ...
    async poll() {
        if (this.running)
            return; // previous poll still in progress — skip
        this.running = true;
        try {
            ...
            let statusMap;
            try {
                statusMap = await getBatchWorkItemStatus(token, ids);
            }
            catch {
            ...
            for (const [workItemId, result] of statusMap.entries()) {
                const daStatus = result.status;
                const mappedState = daStatus === "inprogress" ? "inprogress" :
