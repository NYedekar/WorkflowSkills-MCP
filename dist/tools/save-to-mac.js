...
export const saveToMacSchema = z.object({
    content: z
        .string()
        .describe("The text content to save (JSON, CSV, plain text, Markdown, etc.). " +
    ...
    save_to: z
        ...
        .optional()
        .default("~/Downloads")
        .describe("Target folder on the Mac. Defaults to ~/Downloads. " +
        "~ is expanded to the home directory. The folder is created if it does not exist."),
    filename: z
        .string()
        .describe("Filename including extension (e.g. 'metadata.json', 'report.csv', 'summary.md'). " +
        "If the file already exists it will be overwritten."),
});
export async function handleSaveToMac(input) {
