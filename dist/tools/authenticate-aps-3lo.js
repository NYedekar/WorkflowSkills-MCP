...
export const authenticateAps3LOSchema = z.object({
    scopes: z
        .array(z.string())
        .optional()
        .describe("OAuth scopes to request. Defaults to: account:read, account:write, " +
    ...
    timeout_seconds: z
        ...
        .optional()
        .default(120)
        .describe("Seconds to wait for the user to complete authorization in the browser. Default: 120."),
});
export async function handleAuthenticateAps3LO(input) {
    ...
    await new Promise((res, rej) => {
        server.on("error", (err) => {
            if (err.code === "EADDRINUSE") {
                rej(new Error(`Port ${port} is already in use. ` +
    ...
    try {
        ...
        return {
            status: "success",
            message: "3-legged OAuth complete. Your user token is active and will be used automatically " +
        ...
        return {
            ...
            hint: "Verify that your APS app has this exact redirect URI registered:\n" +
                `  ${REDIRECT_URI}\n` +
