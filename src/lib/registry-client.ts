...
function scoreCapability(c: CapabilityRecord, terms: string[]): number {
  ...
  if (isAccAdminIntent && ACC_ADMIN_CAPS.has(c.id)) score += 25;
  if (isAccAdminIntent && DM_NAV_CAPS.has(c.id)) score -= 10;
  if (isDmNavIntent && DM_NAV_CAPS.has(c.id)) score += 15;
  if (isDmNavIntent && ACC_ADMIN_CAPS.has(c.id)) score -= 5;
